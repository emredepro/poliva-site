  ...
  return (
    <div style={{ backgroundColor: 'black', minHeight: '100vh', color: 'white', overflowX: 'hidden', fontFamily: "'Avant Garde', sans-serif" }}>
        ...
        <main className="main-scroll">
          <section className="hero-section">
            ...
            <div className="citation responsiva anim-fade-up">
              <p>"A música não é apenas entertainment; ela é portal. Não é só sobre tocar música, é sobre atravessá-la."</p>
          ...
          <section className="section-block spacer-lg" style={{ marginBottom: '150px' }}>
             <div className="brutal-header mobile-boost anim-fade-up">
        ...
        <div className="radio-bar">
          <div className="radio-inner">
            <div className="radio-controls">
              <button onClick={() => setCurrentTrack(prev => (prev === 0 ? 3 : prev - 1))} className="radio-nav-btn interactive-zoom">«</button>
              ...
              <button onClick={() => setCurrentTrack(prev => (prev + 1) % 4)} className="radio-nav-btn interactive-zoom">»</button>
            </div>
            <div className="radio-display">
              <div className="marquee-box mar-display"><p className={isPlaying ? 'running' : ''}>{tracks[currentTrack].name}</p></div>
      ...
      <style jsx global>{`
        ...
        .pulse { animation: pulse 2s infinite ease-in-out; pulse { 0%, 100% { opacity: 0.5; } 50% { opacity: 1; } } }
        ...
        /* YOUTUBE: CONTAINER CENTRALIZADO */
        .video-player { width: 100%; max-width: 540px; margin: 60px auto; aspect-ratio: 16/9; background: #000; box-shadow: 0 50px 100px rgba(0,0,0,0.9); transition: 0.4s ease; z-index: 100; position: relative; }
        .video-player iframe { width: 100%; height: 100%; border-radius: 4px; border: none; pointer-events: auto; }
        ...
        .car-track { display: flex; transition: 0.8scubic-bezier(0.23, 1, 0.32, 1); }
        .car-item { min-width: 100%; text-align: center; }
        ...
        /* RODAPÉ AJUSTADO E CURTO */
        .footer-black { background: black !important; padding: 80px 20px 120px; border-top: 1px solid #111; position: relative; z-index: 5; margin-top: 150px; }
        .copyright-line { margin-top: 60px; font-size: 10px; color: #444; }

        /* RADIO: TRAVADA NA BASE */
        ...
        .mar-display { width: 280px; overflow: hidden; white-space: nowrap; margin-bottom: 5px; border-bottom: 1px solid rgba(168, 85, 247, 0.1); padding-bottom: 5px; }
        .running { animation: marquee 15s linear infinite; }
        @keyframes marquee { 0% { transform: translateX(100%); } 100% { transform: translateX(-100%); } }

        .wa-btn-fixed { position: fixed; bottom: 120px; right: 30px; width: 50px; z-index: 8000; transition: 0.3s; }
        .anim-fade-up { opacity: 0; transform: translateY(20px); animation: reveal 0.6s ease forwards; }
